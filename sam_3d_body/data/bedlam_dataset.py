...
class FakeGetBBoxCenterScale(nn.Module):
    """Convert BEDLAM scale format to sam_3d_body format.
    
    BEDLAM scale: scalar value representing height/200 (where height is in pixels)
    sam_3d_body scale: [width, height] in pixels
    
    Since BEDLAM uses square bounding boxes, we convert:
    - BEDLAM scale s -> height = s * 200, width = s * 200
    - sam_3d_body scale = [s * 200, s * 200]
    """
    def __init__(self) -> None:
        super().__init__()

    def forward(self, results: Dict) -> Optional[dict]:
        # BEDLAM scale is a scalar (height/200)
        bedlam_scale = results["scale"]
        
        # Convert to sam_3d_body format: [width, height] in pixels
        # BEDLAM uses square bboxes, so both dimensions are scale * 200
        if np.isscalar(bedlam_scale) or (isinstance(bedlam_scale, np.ndarray) and bedlam_scale.ndim == 0):
            # Single scalar value
            height_pixels = float(bedlam_scale) * 200.0
            sam3d_scale = np.array([height_pixels, height_pixels], dtype=np.float32)
        else:
            # Already an array, but might be single value - convert to [w, h]
            bedlam_scale = np.asarray(bedlam_scale)
            if bedlam_scale.size == 1:
                height_pixels = float(bedlam_scale.item()) * 200.0
                sam3d_scale = np.array([height_pixels, height_pixels], dtype=np.float32)
            else:
                # Assume it's already in [w, h] format, but convert from BEDLAM if needed
                # If it's 2D, assume it's [w, h] already
                sam3d_scale = bedlam_scale.astype(np.float32)
        
        results["bbox_center"] = results["center"].astype(np.float32)
        results["bbox_scale"] = sam3d_scale
        return results
...
class DatasetHMR(Dataset):
    ...
    def __init__(self, options, dataset, use_augmentation=True, is_train=True):
        ...
        if self.is_train:
            if '3dpw-train-smplx' in self.dataset:
                self.pose_cam = self.data['smplx_pose'][:, :NUM_JOINTS_SMPLX*3].astype(float)
    ...
    def rgb_processing(self, rgb_img_full):
        ...
        if self.is_train and self.options.ALB: 
            aug_comp = [A.Downscale((0.5, 0.9), p=0.1),
    ...
    def __getitem__(self, index):
        ...
        keypoints_vis = keypoints[:, 2:3] if keypoints.shape[1] > 2 else np.ones((keypoints.shape[0], 1))
        ...
        item['face_expr_coeffs'] = torch.from_numpy(self.face_expr_params[index]).float()
...
class MultiViewEvaluationDataset(Dataset):
    ...
    def __init__(self, options, dataset, num_view=4, use_augmentation=False, is_train=False):
        ...
        if 'serno' not in self.data.files:
            raise KeyError("'serno' field not found in dataset. This dataset requires serial numbers to group views.")
        ...
        self.valid_sernos = [serno for serno, indices in self.serno_to_indices.items() 
        ...
        logger.info(f'Found {len(self.valid_sernos)} sernos with at least {num_view} views')
    ...
    def _load_single_view(self, index):
        ...
        keypoints_vis = keypoints[:, 2:3] if keypoints.shape[1] > 2 else np.ones((keypoints.shape[0], 1))
        if normalized_keypoints.shape[1] == 2:
            normalized_keypoints = np.hstack([normalized_keypoints.cpu().numpy(), keypoints_vis])
