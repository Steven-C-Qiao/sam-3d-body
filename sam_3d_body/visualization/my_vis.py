...
class Visualiser(pl.LightningModule):
    ...
    def visualise_keypoints_3d(self, gt_keypoints_3d, pred_keypoints_3d, pred_keypoints_3d_samples=None):
        ...
        if pred_keypoints_3d_samples is not None:
            if isinstance(pred_keypoints_3d_samples, torch.Tensor):
                pred_keypoints_3d_samples = pred_keypoints_3d_samples.cpu().detach().numpy()
        ...
        ax1.scatter(gt_kp[:, 0], gt_kp[:, 1], gt_kp[:, 2], 
                   c='blue', marker='o', s=50, alpha=0.6, label='GT Keypoints')
        ax1.set_xlabel('X')
        ax1.set_ylabel('Y')
        ax1.set_zlabel('Z')
        ...
        ax2.scatter(pred_kp[:, 0], pred_kp[:, 1], pred_kp[:, 2], 
                   c='red', marker='^', s=50, alpha=0.6, label='Predicted Keypoints')
        ax2.set_xlabel('X')
        ax2.set_ylabel('Y')
        ax2.set_zlabel('Z')
        ...
        ax3.scatter(gt_kp[:, 0], gt_kp[:, 1], gt_kp[:, 2],
                    c='blue', marker='o', s=40, alpha=0.6, label='GT')
        ax3.scatter(pred_kp[:, 0], pred_kp[:, 1], pred_kp[:, 2],
                    c='red', marker='^', s=40, alpha=0.6, label='Pred')
        ax3.set_xlabel('X')
        ax3.set_ylabel('Y')
        ax3.set_zlabel('Z')
        ...
        if sample_kps is not None:
            for i in range(num_samples):
                ...
                ax.scatter(sample_kp[:, 0], sample_kp[:, 1], sample_kp[:, 2],
                          c='green', marker='s', s=50, alpha=0.6, label=f'Sample {i+1}')
                ax.set_xlabel('X')
                ax.set_ylabel('Y')
                ax.set_zlabel('Z')
                ...
                ax.set_xlim(mid_x_sample - max_range_sample, mid_x_sample + max_range_sample)
                ax.set_ylim(mid_y_sample - max_range_sample, mid_y_sample + max_range_sample)
                ax.set_zlim(mid_z_sample - max_range_sample, mid_z_sample + max_range_sample)
    ...
    def visualise_mesh_pyplot(self, predictions, batch):
        ...
        def set_3d_axes_equal(ax, vertices, title, color, marker='o'):
            ...
            ax.scatter(vertices[:, 0], vertices[:, 1], vertices[:, 2],
                      c=color, marker=marker, s=1, alpha=0.5)
            ax.set_xlabel('X')
            ax.set_ylabel('Y')
            ax.set_zlabel('Z')
            ax.set_title(title)
        ...
        for i in range(num_samples):
            ...
            set_3d_axes_equal(ax, sample_verts, f'Sample {i+1} Vertices', 'green', marker='s')
    ...
    def visualise_mesh(self, predictions, batch):
        ...
        # Render GT mesh
        gt_img = self._render_mesh(gt_verts, faces, cam_t, focal_length, render_size, 
        ...
        pred_img = self._render_mesh(pred_verts, faces, cam_t, focal_length, render_size,
        ...
        for i in range(num_samples):
            sample_verts = pred_verts_samples[i]  # (18439, 3)
            sample_img = self._render_mesh(sample_verts, faces, cam_t, focal_length, render_size,
    ...
    def _render_mesh(self, vertices, faces, cam_t, focal_length, render_size, mesh_color=(0.8, 0.8, 0.8)):
        """
    ...
    def visualise_2d_keypoints(self, predictions, batch):
        ...
        if 'mhr_samples_keypoints_2d' not in predictions:
            logger.warning("No sample keypoints found in predictions. Skipping 2D keypoint visualization.")
            return
        ...
        # Plot GT keypoints
        plt.scatter(gt_kp2d[:, 0], gt_kp2d[:, 1],
                   color='blue', s=15, marker='x', label='GT', linewidths=2)

        # Plot predicted mean keypoints
        plt.scatter(pred_kp2d_cropped_coords[:, 0], pred_kp2d_cropped_coords[:, 1],
                   color='red', s=15, marker='o', label='Pred Mean',
    ...
    def visualise_merging(self, predictions, batch=None, save_path=None, suffix=None, normalise=True):
        ...
        gt_verts_all_views = predictions['gt_vertices']  # [N_views, N_verts, 3] or [B, N_views, N_verts, 3]
            
        pred_verts_all_views = predictions['pred_vertices']  # [N_views, N_verts, 3] or [B, N_views, N_verts, 3]
        ...
        for view_idx in range(num_views):
            ...
            merged_distances = np.linalg.norm(merged_verts - gt_verts, axis=1)  # [N_verts]
        ...
        for view_idx in range(num_views):
            ...
            ax1.scatter(gt_verts[:, 0], gt_verts[:, 1], gt_verts[:, 2], 
                       c='blue', s=1, alpha=0.6, label='GT Vertices')
            ...
            scatter2 = ax2.scatter(pred_verts[:, 0], pred_verts[:, 1], pred_verts[:, 2],
                       c=pred_distances_cm, s=1, alpha=0.6, cmap='viridis', 
                       vmin=0, vmax=global_max_distance_cm, label='Predicted Vertices')
            ax2.set_title(f'View {view_idx+1}: Predicted Vertices\n(Avg distance: {avg_pred_distance_cm:.2f} cm)')
            ax2.set_xlabel('X')
            ax2.set_ylabel('Y')
            ax2.set_zlabel('Z')
            ...
            scatter3 = ax3.scatter(merged_verts[:, 0], merged_verts[:, 1], merged_verts[:, 2],
                       c=merged_distances_cm, s=1, alpha=0.6, cmap='viridis',
                       vmin=0, vmax=global_max_distance_cm, label='Merged Vertices')
            ax3.set_title(f'View {view_idx+1}: Merged Vertices\n(Avg distance: {avg_merged_distance_cm:.2f} cm)')
            ax3.set_xlabel('X')
            ax3.set_ylabel('Y')
            ax3.set_zlabel('Z')
